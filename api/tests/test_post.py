...
class TestPostAPI:
    ...
    def test_posts_get(self, user_client, post, another_post):
        ...
        assert type(test_data) == dict, (
            'Проверьте, что при GET запросе на `/api/v1/posts/` возвращается словарь'
        )
        ...
        test_post = test_data['results'][0]
        assert 'id' in test_post, (
            'Проверьте, что добавили `id` в список полей `fields` сериализатора модели Post'
        )
        assert 'title' in test_post, (
            'Проверьте, что добавили `title` в список полей `fields` сериализатора модели Post'
        )
    ...
    def test_post_create(self, user_client, user, another_user):
        ...
        data = {'author': another_user.id, 'title': 'Статья номер 3'}
        response = user_client.post('/api/v1/posts/', data=data)
    ...
    def test_post_get_current(self, user_client, post, user):
        ...
        test_data = response.json()
        assert test_data.get('title') == post.title, (
    ...
    def test_post_patch_current(self, user_client, post, another_post):
        response = user_client.patch(f'/api/v1/posts/{post.id}/',
                                     data={'title': 'Поменяли текст статьи'})
        ...
        assert test_post.title == 'Поменяли текст статьи', (
